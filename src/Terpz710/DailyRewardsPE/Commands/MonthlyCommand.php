...
class MonthlyCommand extends Command implements PluginOwned {
    ...
    public function __construct(Main $plugin) {
        parent::__construct("monthly", "Claim your monthly items");
        $this->plugin = $plugin;
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool {
        if ($sender instanceof Player) {
            $cooldownKey = "monthly_cooldown_" . $sender->getName();
            $cooldown = $this->cooldowns->get($cooldownKey, 0);

            $currentTimestamp = time();

            if ($cooldown <= $currentTimestamp) {
                $cooldown = $currentTimestamp + 30 * 24 * 60 * 60;
                $this->cooldowns->set($cooldownKey, $cooldown);
                $this->cooldowns->save();

                $rewardsList = $this->getRewardList("items_monthly");

                if (!empty($rewardsList)) {
                    $randomReward = $rewardsList[array_rand($rewardsList)];
                    $this->giveReward($sender, $randomReward);
                } else {
                    $sender->sendMessage($this->messages->get("no-monthly-rewards"));
                }
            } else {
                $remainingTime = $cooldown - $currentTimestamp;
                $this->sendCooldownMessage($sender, "monthly", $remainingTime, "cooldown-message-monthly");
            }
        } else {
    ...
    private function giveReward(Player $player, array $rewardData) {
        if (isset($rewardData['is_bank_note']) && $rewardData['is_bank_note']) {
            ...
            $player->sendMessage($this->messages->get("claimed-monthly-item"));
        }
    }
